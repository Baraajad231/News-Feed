...
function App() {
  ...
  const fetchAndUpdateArticles = (qery, page, type = category.current) => {
    setIsLoading(true);
    setError("");
  ...
  const categoryMenuHandler = (type) => {
    ...
    fetchAndUpdateArticles(queryValue.current, pageNumbers.current, category);
  };
  return (
    <Container>
      <NewsHeader
        onSearchChange={searchChangeHandler}
        onCategoryChange={categoryMenuHandler}
        category={category}
      />
      {error.length === 0 ? (
