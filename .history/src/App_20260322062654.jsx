...
function App() {
  ...
  const typeMenu = useRef("");

  const loadData = async (q, page, category) => {
    // const controller = new AbortController();

    const response = await fetch(
      `https://newsapi.org/v2/top-headlines?country=us&q=${q}&page=${page}&pageSize=5&category=${""}&apiKey=${import.meta.env.VITE_NEWS_API_KEY}`,
    );
  ...
  const fetchAndUpdateArticles = (qery, page, type = typeMenu.current) => {
    setIsLoading(true);
    setError("");
    loadData(qery, page, type)
      .then((data) => {
        setArticles(data);
      })
      .catch((error) => {
