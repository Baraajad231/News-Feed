...
function App() {
  ...
  const loadData = async (q, page, category = typeMenu.current) => {
  ...
  const fetchAndUpdateArticles = (qery, page, category) => {
    setIsLoading(true);
    setError("");
  ...
  const categoryMenuHandler = (type) => {
    ...
    fetchAndUpdateArticles(queryValue.current, pageNumbers.current, type);
  };
  return (
    <Container>
      <NewsHeader
