...
function App() {
  ...
  const loadData = async (q, page, category) => {
    ...
    return data?.articles?.map((article) => {
      const { title, description, urlToImage, author, publishedAt, url } = article;
      return { title, description, author, publishedAt,url image: urlToImage };
    });
  };
  const fetchAndUpdateArticles = (query, page, type) => {
    setIsLoading(true);
    setError("");
    loadData(query, page, type ?? "general")
      .then((data) => {
        setArticles(data);
      })
      .catch((error) => {
